...
bool CWS::begin() {
  ...
  // ADC NTC
  esp_adc_cal_characterize(CWS_ADC_UNIT, CWS_ADC_ATTEN, CWS_ADC_WIDTH, 1125, &_adc_chars_ntc);
  adc2_config_channel_atten(ADC2_CHANNEL_3, CWS_ADC_ATTEN);

  // ADC Light
  esp_adc_cal_characterize(CWS_ADC_UNIT, CWS_ADC_ATTEN, CWS_ADC_WIDTH, 1125, &_adc_chars_light);
  adc2_config_channel_atten(ADC2_CHANNEL_5, CWS_ADC_ATTEN);
...
bool CWS::getData(float &temperature, float &ntc_voltage, float &illuminance, float &light_voltage, float &x, float &y, float &z, float &magnitude, float &inclination) {
  temperature = _readTemperature();
  ntc_voltage = _readNTCVoltage();
...
bool CWS::getJSON(JsonDocument &doc) {
  float temperature = _readTemperature();
  float ntc_voltage = _readNTCVoltage();
  ...
  dataSet["value"] = temperature;
  dataSet["unit"] = "ºC";

  dataSet = dataArray.add<JsonObject>();  // Subsequent data sets
  dataSet["name"] = "NTC Voltage";
  dataSet["value"] = ntc_voltage;
...
float CWS::_readTemperature() {
  float voltage = _readNTCVoltage();
  ...
  float tempK = 1.0f / (CWS_SH_A + CWS_SH_B * lnR + CWS_SH_C * lnR * lnR * lnR);
  return tempK - 273.15f;
}

float CWS::_readNTCVoltage() {
  return _readADC(ADC2_CHANNEL_3);
}
...
float CWS::_readLightVoltage() {
  return _readADC(ADC2_CHANNEL_5);
}
...
float CWS::_readADC(uint8_t channel) {
  ...
  for (int i = 0; i < CWS_SAMPLES; i++) {
    int raw;
    if (adc2_get_raw((adc2_channel_t)channel, CWS_ADC_WIDTH, &raw) == ESP_OK) sum += raw;
    delay(1);
  }
  int avg_raw = sum / CWS_SAMPLES;
  if (avg_raw < 0 || avg_raw > 8191) return 0.0f;
  return (float)esp_adc_cal_raw_to_voltage(avg_raw, channel == ADC2_CHANNEL_3 ? &_adc_chars_ntc : &_adc_chars_light);  // Returns mV directly
}